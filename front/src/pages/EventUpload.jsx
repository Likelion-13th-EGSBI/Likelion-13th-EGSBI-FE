...
// 좌표를 이용해서 행정구역(동) 정보를 가져오는 함수
const getDongFromCoordinates = async (latitude, longitude) => {
return new Promise((resolve, reject) => {
  ...
  // coord2Address 호출 (경도, 위도 순서 주의)
  geocoder.coord2Address(longitude, latitude, (result, status) => {
    console.log('카카오 Geocoder 결과:', result, '상태:', status);
    
    if (status === window.kakao.maps.services.Status.OK) {
      // result[0].address에 행정구역 정보가 있음
      const addressInfo = result[0]?.address;
      if (addressInfo) {
        console.log('주소 정보:', addressInfo);
        // region_3depth_name이 동/읍/면 정보 (실제 행정구역)
        const dong = addressInfo.region_3depth_name;
        console.log('추출된 실제 행정구역:', dong);
        
        if (dong && dong.trim()) {
          resolve(dong.trim());
        } else {
          reject(new Error('행정구역 정보를 찾을 수 없습니다.'));
        }
      } else {
        reject(new Error('주소 정보를 찾을 수 없습니다.'));
      }
    } else {
      console.error('좌표→주소 변환 실패, 상태:', status);
      reject(new Error(`좌표에서 주소 변환에 실패했습니다. (상태: ${status})`));
    }
  });
});
};
...
const [locationError, setLocationError] = useState(''); // 위치 오류 메시지
...
const fetchUserLocation = async (userId, accessToken) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(`사용자 위치 조회 실패 (${response.status})`);
    }
    ...
    console.error('사용자 위치 조회 오류:', error);
    throw error;
  }
};

// 위치 검증 함수 - 좌표 기반으로 수정
const validateEventLocation = async (selectedAddress) => {
  try {
    // 사용자 위치 정보가 없으면 조회
    if (!userLocation) {
      ...
      // 사용자 위치의 좌표로 "동" 정보 가져오기
      const userDong = await getDongFromCoordinates(userData.latitude, userData.longitude);
      if (!userDong) {
        alert('사용자 위치의 동 정보를 확인할 수 없습니다.');
        throw new Error('사용자 위치의 동 정보를 확인할 수 없습니다.');
      }
      
      // 선택한 주소의 좌표로 "동" 정보 가져오기
      const eventDong = await getDongFromCoordinates(selectedAddress.latitude, selectedAddress.longitude);
      if (!eventDong) {
        alert('선택한 주소의 동 정보를 확인할 수 없습니다.');
        throw new Error('선택한 주소의 동 정보를 확인할 수 없습니다.');
      }
      
      // 동이 다른 경우 에러
      if (userDong !== eventDong) {
        alert(`내가 설정한 위치랑 다릅니다. "${userDong}" 안에서 행사 개최 위치를 설정해주세요`);
        throw new Error(`내가 설정한 위치랑 다릅니다. "${userDong}" 안에서 행사 개최 위치를 설정해주세요`);
      }
      
      return { isValid: true, userDong, eventDong };
    } else {
      // 이미 사용자 위치가 있는 경우
      const userDong = await getDongFromCoordinates(userLocation.latitude, userLocation.longitude);
      const eventDong = await getDongFromCoordinates(selectedAddress.latitude, selectedAddress.longitude);
...
// 컴포넌트 마운트시 사용자 위치 정보 조회 및 동 정보 추출
useEffect(() => {
  const loadUserLocationAndDong = async () => {
    try {
      const userId = localStorage.getItem('userId');
  ...
  if (kakaoReady) {
    loadUserLocationAndDong();
  }
}, [accessToken, kakaoReady]);
...
const handleAddressSelect = async (selectedAddress) => {
  ...
  try {
    // 위치 검증 수행
    const validation = await validateEventLocation(selectedAddress);
    ...
    console.log('주소 설정 완료:', {
      ...
      latitude: selectedAddress.latitude,
      longitude: selectedAddress.longitude,
      userDong: validation.userDong,
      eventDong: validation.eventDong
    });
    
  } catch (error) {
    console.error('주소 선택 처리 오류:', error);
    setLocationError(error.message || '주소 설정 중 오류가 발생했습니다.');
  }
};
...
const canProceed = () => {
  ...
  switch (step.id) {
    ...
    case 'fee':
      return formData.fee !== '';
    case 'hashtags':
      return selectedMode === 'ai' ? true : formData.hashtags.length > 0;
    case 'description':
        ...
        {currentStepData.type === 'address' && (
          <div className="eventupload-input-group">
            <div className="eventupload-address-container">
              <input
                className={`eventupload-input ${locationError ? 'error' : ''}`}
                type="text"
                placeholder={
                  kakaoReady
              ...
              {locationError && (
                <div className="eventupload-location-error">
                  <span className="eventupload-error-icon">⚠️</span>
                  <span className="eventupload-error-text">{locationError}</span>
                </div>
              )}
              
              {userLocation && (
                <div className="eventupload-user-location-info">
                  <span className="eventupload-location-icon">📍</span>
                  <span className="eventupload-location-text">
                    내 설정 위치: {userLocation.dong || '조회 중...'}
                  </span>
                </div>
              )}
